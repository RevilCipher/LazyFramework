...
class ThemeManager:
    def __init__(self, app, main_window):
        ...
        # === PERUBAHAN: Set default ke dark.qss ===
        self.DEFAULT_THEME_FILE = "dark.qss"
        
        # Load dari config, atau default ke dark.qss
        self.current_theme = self._load_from_config()
        
        # Jika tidak ada theme yang tersimpan atau theme tidak valid, gunakan dark.qss
        if not self.current_theme or self.current_theme not in self.theme_map.values():
            # Cek apakah dark.qss ada di theme_map
            if self.DEFAULT_THEME_FILE in self.theme_map.values():
                self.current_theme = self.DEFAULT_THEME_FILE
            elif self.theme_map:
                # Fallback ke theme pertama yang ditemukan
                self.current_theme = list(self.theme_map.values())[0]
    ...
    def _load_from_config(self):
        ...
        # Jika tidak ada config, kembalikan None (akan menggunakan default)
        return None
