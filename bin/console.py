...
CONFIG_FILE = BASE_DIR / "config.json"  # TAMBAHKAN INI
...
ZENMUX_URL = "https://zenmux.ai/api/v1"  # TAMBAHKAN
...
class CLIAssistant:
    ...
    Pakai: ai <pertanyaan>   → tanya langsung
           ...
           ai key <sk-..>    → set API key
           ai model <name>   → ganti model
           ai provider       → ganti provider (openrouter|zenmux)
    """
    ...
    def __init__(self):
        ...
        self._smart:         bool       = True
        self._context_max:   int        = 8000
        
        # Load saved config
        self._load_config()
        
        # Set provider & model from config
        self._provider = self._config.get("ai_provider", self.DEFAULT_PROVIDER)
        self._model = self._config.get("ai_model", self.DEFAULT_MODEL)
        self._api_key = self._config.get("ai_api_key", os.environ.get("OPENROUTER_API_KEY", ""))
    ...
    def _save_config(self):
        ...
        try:
            ...
            # Update AI settings
            config["ai_provider"] = self._provider
            config["ai_model"] = self._model
            config["ai_api_key"] = self._api_key
            
            with open(CONFIG_FILE, 'w') as f:
                json.dump(config, f, indent=2)
        except Exception as e:
            console.print(f"[dim]Could not save config: {e}[/dim]")
    ...
    def handle(self, args: list[str]) -> bool:
        ...
        if sub == "provider":
            return self._cmd_provider(args[1:])
        if sub == "ctx":
    ...
    def _cmd_model(self, args):
        if not args:
            ...
            console.print(f"[cyan]Provider:[/cyan] {self._provider}")
            return True
    ...
    def _cmd_provider(self, args):
        if not args:
            console.print(f"[cyan]Provider aktif:[/cyan] {self._provider}")
            console.print(f"[dim]Available providers: {', '.join(self.PROVIDERS.keys())}[/dim]")
            return True
        
        provider = args[0].lower()
        if provider not in self.PROVIDERS:
            console.print(f"[red]Provider '{provider}' tidak dikenal.[/red]")
            console.print(f"[dim]Available: {', '.join(self.PROVIDERS.keys())}[/dim]")
            return True
        
        self._provider = provider
        # Auto-set default model untuk provider
        self._model = self.PROVIDERS[provider]["default_model"]
        self._save_config()
        console.print(f"[green]✓ Provider → {self._provider}[/green]")
        console.print(f"[green]✓ Model → {self._model}[/green]")
        return True
    ...
    def _show_help(self):
        console.print(
            ...
            "  [cyan]ai provider [openrouter|zenmux][/cyan]  switch provider (default: openrouter)\n"
            "  [cyan]ai key <sk-...>[/cyan]                  set API key (auto-saved)\n"
            "  [cyan]ai model [name][/cyan]                  set / lihat model (auto-saved)\n\n"
    ...
    # ── Core: kirim ke provider ────────────────────────────────────

    def _ask(self, question: str):
        if not self._api_key:
            console.print("[red]API key belum diset. Gunakan: ai key <sk-...>[/red]")
            return

        # Get provider URL
        provider_config = self.PROVIDERS.get(self._provider, self.PROVIDERS["openrouter"])
        ...
        console.print(f"[dim]LazyAI ({self._model} via {self._provider}) ...[/dim]")
        ...
        try:
            ...
            console.print("[red]Tidak bisa terhubung ke provider. Cek koneksi.[/red]")
        except requests.exceptions.HTTPError as e:
            code = e.response.status_code if e.response else "?"
            msgs = {
                401: "API key tidak valid (401).",
                402: "Kredit habis (402). Top up di provider.",
...
# ... (rest of console.py - LazyFramework class and main unchanged)
...
class LazyFramework:
    ...
    def scan_modules(self):
        ...
        if not MODULE_DIR.exists():
            self.console.print(f"[red]Modules directory not found: {MODULE_DIR}[/red]")
            return
        ...
        if self.modules:
            self.console.print(f"[bold green][*][/bold green] Scanning module tree... {len(self.modules)} module(s)")
        else:
            self.console.print(f"[red]No modules found in {MODULE_DIR}[/red]")
        ...
        try:
            ...
            if _mod_orig_console is not None:
                class _ModTeeConsole:
                    def print(self, *args, **kwargs):
                        _mod_orig_console.print(*args, **kwargs)
                        plain = " ".join(str(a) for a in args) + "\n"
                        ai_buf.write(plain)
                    def __getattr__(self, name):
    ...
    def cmd_help(self, args):
        commands = [
            ...
            ("ai key <sk->", "Set API key (auto-saved)"),
            ("ai model <name>", "Switch AI model (auto-saved)"),
            ("ai provider <openrouter|zenmux>", "Switch AI provider (auto-saved)"),
    ...
    # ... (rest of methods: cmd_payloads, _show_modules_by_type, etc. unchanged)

    def cmd_show(self, args):
        if not args:
            console.print("[bold red][!][/bold red] Argument Failed")
            console.print("[bold green][✓][/bold green] Valid parameters: show modules|recon|scan|exploit|post|privesc|persistence|lateral|web|cloud|mobile|aux|payload|evasion|report")
            return
        
        subcommand = args[0].lower()
        
        valid_types = ['recon', 'scan', 'exploit', 'post', 'privesc', 'persistence', 
                   'lateral', 'web', 'cloud', 'mobile', 'aux', 'payload', 'evasion', 'report']
        ...
        if subcommand == "modules":
        ...
        elif subcommand in valid_types:
            ...
            console.print(f"Usage: show {'|'.join(valid_types)}", style="yellow")
    ...
    def _show_modules_by_type(self, module_type):
        ...
        type_descriptions = {
            ...
            'scan': '🔍 Scanning & Discovery Modules',
            'exploit': '⚡ Exploitation Modules',
            'post': '🎯 Post-Exploitation Modules',
            'privesc': '⬆️ Privilege Escalation Modules',
            'persistence': '🔒 Persistence Modules',
            'lateral': '🔄 Lateral Movement Modules',
            'web': '🌐 Web Application Modules',
            'cloud': '☁️ Cloud Security Modules',
            'mobile': '📱 Mobile Security Modules',
            'aux': '🛠️ Auxiliary Modules',
            'payload': '💣 Payload Generation Modules',
            'evasion': '👻 Evasion & Bypass Modules',
    ...
    def _show_available_types(self):
        ...
        type_descriptions = {
            ...
            'scan': '🔍 Network and service scanning & discovery',
            'exploit': '⚡ Vulnerability exploitation',
            'post': '🎯 Post-exploitation operations',
            'privesc': '⬆️ Privilege escalation techniques',
            'persistence': '🔒 Maintaining access',
            'lateral': '🔄 Lateral movement across network',
            'web': '🌐 Web application security testing',
            'cloud': '☁️ Cloud platform security',
            'mobile': '📱 Mobile application testing',
            'aux': '🛠️ Auxiliary utilities & helpers',
            'payload': '💣 Payload generation & delivery',
    ...
    def _get_available_types(self):
        ...
        for key in self.modules.keys():
            if "/recon/" in key.lower():
                types_count['recon'] += 1
            if "/scan/" in key.lower():
                types_count['scan'] += 1
            if "/exploit/" in key.lower():
                types_count['exploit'] += 1
            if "/post/" in key.lower():
                types_count['post'] += 1
            if "/privesc/" in key.lower():
                types_count['privesc'] += 1
            if "/persistence/" in key.lower():
                types_count['persistence'] += 1
            if "/lateral/" in key.lower():
                types_count['lateral'] += 1
            if "/web/" in key.lower():
                types_count['web'] += 1
            if "/cloud/" in key.lower():
                types_count['cloud'] += 1
            if "/mobile/" in key.lower():
                types_count['mobile'] += 1
            if "/aux/" in key.lower():
                types_count['aux'] += 1
            if "/payload/" in key.lower():
                types_count['payload'] += 1
            if "/evasion/" in key.lower():
                types_count['evasion'] += 1
            if "/report/" in key.lower():
                types_count['report'] += 1
    ...
    def repl(self):
        ...
        commands = ['use', 'set', 'run', 'show', 'info', 'search', 'back', 'help', 'exit',
                    'ai', 'ai key', 'ai model', 'ai provider', 'ai ctx', 'ai clear', 'ai smart', 'ai history',
                    'ai prompts',
